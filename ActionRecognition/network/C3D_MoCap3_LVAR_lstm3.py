...
class C3D_MoCap(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False):
        ...
        self.hidden_size = 19
        self.num_layers = 1
        self.lstm = nn.LSTM(19 * 4, self.hidden_size, self.num_layers, batch_first=True)  # input_size,hidden_size ,num_layers

        self.fc7 = nn.Linear(4096, 512)
        self.fc8 = nn.Linear(512 + 19, 512 + 19)  # 特征层融合
        self.fc9 = nn.Linear(512 + 19, num_classes)
    ...
    def forward(self, x, input_mocap_tensor, x_pre0,x_pre1,x_pre2,x_pre3,x_pre4, h,c):
        ...
        # todo 我在此处修改了MoCap数据的输入，改为了在之前的基础上间隔一组取数据，取8组。
        # input_mocap_tensor = input_mocap_tensor.view(-1, 1, 16*19) # 注意lstm的输入我在init中定义了是batch first，
        if input_mocap_tensor.shape[1] >= 15:
            input_mocap_tensor = input_mocap_tensor[:, [0, 2, 4, 6, 8, 10, 12, 14]]
        input_mocap_tensor = input_mocap_tensor.view(-1, 1, 4 * 19)
        input_mocap_tensor, (h, c) = self.lstm(input_mocap_tensor, (h, c))
        # print(h[0][0][0],c[0][0][0])
        # # 直接拼接concatenate:
        input_mocap_tensor = input_mocap_tensor.view(-1, 19)
...
# todo 改get_1x_lr_params和get_10x_lr_params中层
def get_1x_lr_params(model):
    ...
    b = [model.conv1, model.conv2, model.conv3a, model.conv3b, model.conv4a, model.conv4b,
         model.conv5a, model.conv5b, model.fc6, model.fc7, model.lstm, model.fc8]
...
if __name__ == "__main__":
    ...
    h0 = torch.zeros(1, input_mocap_tensor.size(0), 19).clone().detach()
    c0 = torch.zeros(1, input_mocap_tensor.size(0), 19).clone().detach()
    ...
    FLOPs, params = profile(net, (inputs1, input_mocap_tensor,x_pre0,x_pre1,x_pre2,x_pre3,x_pre4, h, c))
    print(FLOPs)  # 462 GFLOPs  (G:10E9)
    print(params)  # 62.357MParams  (M:10E6)
    print(outputs.size())
    print(outputs)