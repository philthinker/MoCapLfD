...
class C3D_MoCap(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False, LSTM_pretrained=False):
        ...
        self.fc7 = nn.Linear(4096+self.hidden_size, 4096+self.hidden_size)  # 特征层融合
        self.fc8 = nn.Linear(4096+self.hidden_size, num_classes)
    ...
    def forward(self, x, input_mocap_tensor, x_pre0,x_pre1,x_pre2,x_pre3,x_pre4, h,c, feature_out=False):
        ...
        # todo: 保存特征值
        if feature_out:
            feature = x.detach().cpu().squeeze(0).numpy()
            # numpy.save("testnpy", feature)

        x = self.dropout(x)
        logits = self.fc8(x)

        if feature_out:
            return logits, x_pre0, x_pre1, x_pre2, x_pre3, x_pre4, h, c, feature
        else:
...
if __name__ == "__main__":
    ...
    print(FLOPs)  # 462.847993824 GFLOPs  (G:10E9)
    print(params)  # 78.314988MParams  (M:10E6)
    print(outputs.size())
    print(outputs)