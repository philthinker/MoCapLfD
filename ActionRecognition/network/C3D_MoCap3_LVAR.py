...
class C3D_MoCap(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False):
        ...
        self.fc7 = nn.Linear(4096 + 19 * 16, 4096 + 19 * 16)  # 特征层融合
        self.fc8 = nn.Linear(4096 + 19 * 16, num_classes)
    ...
    def forward(self, x, input_mocap_tensor, x_pre0,x_pre1,x_pre2,x_pre3,x_pre4):

        # 错了， 这里拼接的应该是现在这16帧对应数据与之前16帧对应数据:怎么解决：输入x_o不再为16帧，而是clip_len=18帧，将其拆分为前16帧与后16帧  ???---No!!!

        # torch.Size([1, 3, 16, 112, 112])依次为batchsize=1，rgb3个通道，时间序列上为16帧，裁剪图大小为112*112；
        # 即batchsize，channel通道数，一次的序列长度（时间维度），crop的长宽h、w）

        # tmp = x_pre
        # tmp.clear()
        # tmp.append(x.index_select(2, torch.tensor([x.shape[2] - 2, x.shape[2] - 1])).clone().detach())
        ...
        return logits, x_pre0,x_pre1,x_pre2,x_pre3,x_pre4
...
if __name__ == "__main__":
    inputs1 = torch.rand(11, 3, 16, 112, 112)
    ...
    x_pre1 = torch.zeros(13, 64, 2, 56, 56).clone().detach()
    ...
    outputs, x_pre0,x_pre1,x_pre2,x_pre3,x_pre4 = net.forward(inputs1, input_mocap_tensor, x_pre0,x_pre1,x_pre2,x_pre3,x_pre4)

    FLOPs, params = profile(net, (inputs1, input_mocap_tensor,x_pre0,x_pre1,x_pre2,x_pre3,x_pre4))
    print(FLOPs)  # 38.572955680 GFLOPs  (G:10E9)
    print(params)  # 80.605270MParams  (M:10E6)
    print(outputs.size())
    print(outputs)