...
class AttLayer(nn.Module):
    ...
    def construct_window_mask(self):
        ...
        for i in range(self.bl):
            # todo： 修改了原先错误的window_mask[:, :, i:i+self.bl] = 1为下面的，有可能导致
...
class Encoder(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate, att_type, alpha):
        ...
        self.conv_1x1 = nn.Conv1d(input_dim, num_f_maps, 1)  # fc layer:输入in_channels=input_dim，输出out_channels=num_f_maps，kernel_size=1
        self.layers = nn.ModuleList(
    ...
    def forward(self, x, mask):
        ...
        if self.channel_masking_rate > 0:
            x = x.unsqueeze(2)  # 在tensor的某个维度上添加一个维数为1的维度，如原先x为【1，2048，4113】，则变为【1，2048，1，4113】
            x = self.dropout(x)  #self.dropout = nn.Dropout2d(p=channel_masking_rate)，与self.channel_masking_rate有关
            x = x.squeeze(2)  # 压缩一个tensor的维数为1的维度，使该tensor降维变成最紧凑的形式.如x从【1，2048，1，4113】变为【1，2048，4113】

        feature = self.conv_1x1(x)  # 全连接层，用于调整输入特征的尺寸。若x为【1，2048，4113】，则feature为【1，64(即num_f_maps)，4113】,4113为帧数
        for layer in self.layers:
            feature = layer(feature, None, mask)
        
        out = self.conv_out(feature) * mask[:, 0:1, :]  # 全连接层，输出预测值（属于R^（T*C）,T表示时间帧数，C表示动作类数）
...
class Decoder(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, att_type, alpha):
        # numlayers = 10, r1=2, r2=2, num_f_maps=64, input_dim=num_classes, att_type='sliding_att', alpha=exponential_descrease(s)
...
class Trainer:
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, batch_gen_tst=None):
        ...
        # todo 自加:
        self.model.load_state_dict(torch.load(r'C:\YHY\Python_code\HY_ActionRec\ASFormer_main\models\Assembly0728\split_1\epoch-50.model'))
        # self.model.load_state_dict(torch.load(r'C:\YHY\Python_code\HY_ActionRec\ASFormer_main\models\Assembly0728\split_1\epoch-50.model'))
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                # batch_target:真实值Size([1, 4718])； batch_input:输入Size([1, 2048, 4718]),2048为一帧的特征值数，4718为帧数
                ...
                loss = 0
                for p in ps:  # 依次求encoder和三个decoder的预测结果与真实值的CrossEntropyLoss
                    loss += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
                    loss += 0.15 * torch.mean(torch.clamp(
                        self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)), min=0,
                        max=16) * mask[:, :, 1:])   # clamp()把一个值限制在一个上限和下限之间（当这个值超过最小值和最大值的范围时，在最小值和最大值之间选择一个值使用）

                epoch_loss += loss.item()  # .item()方法 是得到一个元素张量里面的元素值
                loss.backward()
                optimizer.step()

                # max函数会返回两个tensor，第一个tensor是最大值；第二个tensor是最大值索引。
                _, predicted = torch.max(ps.data[-1], 1)  #ps.data[-1]是最后一个decoder的输出，即最终预测的概率，大小为[1, 19, 4227]；函数中1表示取维度1中的最大值，这里是取19个数中的最大值，最终得到【1, 4227】。
...
#                 print(vid)
    ...
    def predict_save(self, results_dir, actions_dict, sample_rate, features_path, datanames, modelpath = r'C:\YHY\Python_code\HY_ActionRec\ASFormer_main\models\Assembly0728\split_1\epoch-50.model'):
        """
        识别分类并保存识别结果。
        :param results_dir:  输出所在文件夹，'./results/Assembly0728/split_1'
        :param actions_dict: 标签名称与序号 {'Approaching': 0, 'Insertion_R': 1, 'Insertion_T': 2, 'Preparation': 3, 'Press_Horizontal': 4, 'Press_Vertical': 5}
        :param sample_rate: 采样率，一般设置为1
        :param features_path:  输入特征所在文件夹 './data/Assembly0728/features/'
        :param datanames: 输入文件名称['cr5_3', 'cr5_4']
        :param modelpath: 模型参数地址
        :return:
        """
        import csv
        self.model.eval()
            ...
            for fi in range(len(datanames)):
                ...
                for i in range(len(predictions)):
                    ...
                    for j in range(len(predicted)):
                        recognition = np.concatenate((recognition, [list(actions_dict.keys())[
                                                                        list(actions_dict.values()).index(
                                                                            predicted[j].item())]] * sample_rate))
                    f_res = results_dir + "/" + f_name + "_" + str(i) + '.csv'
                    f = open(f_res, 'w', newline='')
                    writer = csv.writer(f)
                    for k in range(len(confidence)):
                        writer.writerow(np.hstack((confidence[k].item(), predicted[k].item(), recognition[k]))) #.tolist()
                    f.close()
